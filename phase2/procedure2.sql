...
CREATE PROCEDURE optionals_question (IN mng int, IN from_date DATETIME,IN to_date DATETIME)
       BEGIN
              SELECT am.questionid AS QID, op.number AS opt, op.txt
              From a_multi_choice as am INNER JOIN options as op on am.questionid = op.questionid INNER JOIN answer as ans on ans.id = am.answerid
              where am.answerid in (
                     ...
                     where sr.managerid = mng AND an.createtion_date BETWEEN from_date AND to_date AND an.ticketid = ans.ticketid
